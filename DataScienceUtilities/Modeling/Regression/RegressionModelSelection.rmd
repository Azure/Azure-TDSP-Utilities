...
list.of.packages <- c('glmnet', 'yaml', 'randomForest', 'xgboost', 'lattice', 'shiny', 'gridExtra','lme4','RODBC','rminer', 'dpSNOW')
...
# Fit glmnet model
if (runGlmnet == TRUE) {
  netFit <- caret::train(trainFormula, data = trainData, family = glmnetFamily, method = "glmnet", standardize = glmnetStandardize, tuneGrid = eGrid,  trControl = controlObject, metric = evaluationMetric); 
...
if (runGlmnet == TRUE) {
  ...
  legend("bottomright", inset = 0.01, legend=paste0("R2: ", round(rminer::mmetric(x=predictions[,targetCol], y=predictions$glmnet, metric='R2'),2)), cex=1, box.lty=0)
}
...
if (runRf == TRUE) {
  ...
  legend("bottomright", inset = 0.01, legend=paste0("R2: ", round(rminer::mmetric(x=predictions[,targetCol], y=predictions$rf, metric='R2'),2)), cex=1, box.lty=0)
}
...
if (runXgBoost == TRUE) {
  ...
  legend("bottomright", inset = 0.01, legend=paste0("R2: ", round(rminer::mmetric(x=predictions[,targetCol], y=predictions$xgb, metric='R2'),2)), cex=1, box.lty=0)
}
```
